  ...
  useEffect(() => {
    ...
    const loadAtsData = async () => {
      ...
      try {
        let res: any = null;
        try {
          res = await ApiService.getLatestAtsAnalysis(user?.uid || "", resume.id);
        } catch {
          // If no existing analysis record found, run quality analysis
          res = await ApiService.analyzeQuality(resume.id, user?.uid || "", resume.parsedData);
        }
        if (!mounted) return;
        setAtsAnalysis(res);
        ...
        if (!mounted) return;
        // Do not display red alert box — let widget render friendly empty state
        setAtsError(null);
      } finally {
            ...
            <div className="space-y-8 animate-fade-in relative z-10">
              ...
              {activeTab === "overview" && (
                <div className="space-y-8">
                  ...
                  {/* Premium Career Board Dashboard Grid */}
                  <div className="space-y-6">
                    {/* Top Row: 3 Equal-Width Columns on Desktop */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
                      ...
                      <div className="glass-card p-6 space-y-4 relative overflow-hidden flex flex-col justify-between">
                        ...
                        <div>
                          <h3 className="text-xs font-mono text-[var(--color-text-secondary)] uppercase tracking-wider font-black border-b border-[var(--color-border)] pb-2.5 mb-4">
                            Candidate Identity
                          </h3>
                        ...
                        {resume?.parsedData?.summary && (
                          <div className="pt-3 border-t border-[var(--color-border)]/50 mt-4">
                            ...
                            <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed font-sans font-medium line-clamp-6 whitespace-pre-line">
                              {resume.parsedData.summary}
                            </p>
                          </div>
                        )}
                      ...
                      {/* Column 2: ATS Resume Score Widget */}
                      {(() => {
                        ...
                        const catMap: Record<string, { label: string; key: string; max: number }> = {
                          formatting:    { label: "Structure",      key: "formatting",    max: 20 },
                          ...
                          keywords:      { label: "Keywords",       key: "keywords",      max: 10 },
                        ...
                        const strengths: string[] = Array.isArray(atsAnalysis?.strengths) ? atsAnalysis.strengths.slice(0, 2) : [];
                        const warnings: string[] = [
                          ...(Array.isArray(atsAnalysis?.missingKeywords) && atsAnalysis.missingKeywords.length > 0
                            ? [`Missing ${atsAnalysis.missingKeywords.length} ATS keyword${atsAnalysis.missingKeywords.length > 1 ? "s" : ""}`]
                            : []),
                          ...(Array.isArray(atsAnalysis?.recommendations)
                            ? atsAnalysis.recommendations.slice(0, 1).map((r: any) => typeof r === "string" ? r : r?.text ?? "")
                            : [])
                        ...
                        return (
                          <div className="glass-card p-5 space-y-4 relative overflow-hidden flex flex-col justify-between">
                            ...
                            {!atsLoading && score === undefined && (
                              <div className="flex flex-col items-center justify-center py-6 space-y-3 text-center my-auto">
                                ...
                                <div>
                                  <p className="text-[11px] font-bold text-[var(--color-text-primary)]">No ATS Analysis Available</p>
                                ...
                                <button
                                  ...
                                  className="px-4 py-2 clay-btn clay-btn-primary text-[10px] font-mono uppercase tracking-wider font-bold text-white cursor-pointer"
                                >
                                  Analyze Resume
                                </button>
                              </div>
                            ...
                            {!atsLoading && score !== undefined && (
                              <div className="space-y-3.5 flex-1 flex flex-col justify-between">
                                <div className={`flex items-center space-x-3.5 p-3 rounded-2xl bg-gradient-to-br border ${scoreBg}`}>
                                  <CircularScoreGauge
                                    score={score}
                                    size={72}
                                    strokeWidth={6}
                                  ...
                                  <div className="flex-1 min-w-0 space-y-1.5">
                                    <div>
                                      <span className="text-[8.5px] font-mono text-[var(--color-text-tertiary)] uppercase font-semibold block">Resume Health</span>
                                      <span className={`text-xs font-black tracking-tight ${scoreText}`}>{health}</span>
                                    </div>
                                    <div>
                                      <span className="text-[8.5px] font-mono text-[var(--color-text-tertiary)] uppercase font-semibold block">Resume Rank</span>
                                ...
                                {categories.length > 0 && (
                                  <div className="space-y-1.5">
                                    <span className="text-[8.5px] font-mono text-[var(--color-text-tertiary)] uppercase tracking-wider font-black block">Score Breakdown</span>
                                    <div className="space-y-1">
                                      {categories.map((cat, i) => (
                                        <div key={i} className="space-y-0.5">
                                          <div className="flex justify-between items-center text-[9px] font-mono">
                                            <span className="font-semibold text-[var(--color-text-secondary)] truncate">{cat.label}</span>
                                ...
                                {(strengths.length > 0 || warnings.length > 0) && (
                                  <div className="space-y-1">
                                    <span className="text-[8.5px] font-mono text-[var(--color-text-tertiary)] uppercase tracking-wider font-black block">AI Resume Insights</span>
                                    <div className="space-y-1">
                                      {strengths.map((s, i) => (
                                        <div key={`s-${i}`} className="flex items-start space-x-1.5">
                                          <CheckCircle2 className="w-3 h-3 text-emerald-500 shrink-0 mt-0.5" />
                                          <span className="text-[9.5px] font-sans font-medium text-[var(--color-text-secondary)] leading-tight">{s}</span>
                                        </div>
                                      ))}
                                      {warnings.map((w, i) => (
                                        <div key={`w-${i}`} className="flex items-start space-x-1.5">
                                          <AlertCircle className="w-3 h-3 text-amber-500 shrink-0 mt-0.5" />
                                          <span className="text-[9.5px] font-sans font-medium text-[var(--color-text-secondary)] leading-tight">{w}</span>
                                        </div>
                                      ))}
                                    </div>
                                  </div>
                                ...
                                <div className="grid grid-cols-2 gap-1.5 pt-1 border-t border-[var(--color-border)]/50">
                                  <button
                                    onClick={() => setActiveTab("ats")}
                                    className="flex items-center justify-center space-x-1 px-2 py-1.5 rounded-xl bg-[#6D5DF6]/5 border border-[#6D5DF6]/15 hover:bg-[#6D5DF6]/10 text-[#6D5DF6] transition duration-150 cursor-pointer"
                                  >
                                    <FileText className="w-3 h-3 shrink-0" />
                                    <span className="text-[8.5px] font-mono font-bold truncate">Full ATS Report</span>
                                  </button>
                                  <button
                                    onClick={() => setActiveTab("roadmap")}
                                    className="flex items-center justify-center space-x-1 px-2 py-1.5 rounded-xl bg-amber-500/5 border border-amber-500/15 hover:bg-amber-500/10 text-amber-500 transition duration-150 cursor-pointer"
                                  >
                                    <TrendingUp className="w-3 h-3 shrink-0" />
                                    <span className="text-[8.5px] font-mono font-bold truncate">Improve Resume</span>
                                  </button>
                                  <button
                                    onClick={triggerQualityAnalysis}
                                    disabled={analyzingQuality}
                                    className="flex items-center justify-center space-x-1 px-2 py-1.5 rounded-xl bg-[var(--color-bg-page)] border border-[var(--color-border)] hover:border-[#6D5DF6]/20 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition duration-150 cursor-pointer disabled:opacity-50"
                                  >
                                    <RefreshCw className={`w-3 h-3 shrink-0 ${analyzingQuality ? "animate-spin" : ""}`} />
                                    <span className="text-[8.5px] font-mono font-bold truncate">{analyzingQuality ? "Analyzing..." : "Analyze Again"}</span>
                                  </button>
                                  <button
                                    onClick={() => {
                                    ...
                                    className="flex items-center justify-center space-x-1 px-2 py-1.5 rounded-xl bg-emerald-500/5 border border-emerald-500/15 hover:bg-emerald-500/10 text-emerald-500 transition duration-150 cursor-pointer"
                                  >
                                    <ArrowUpRight className="w-3 h-3 shrink-0" />
                                    <span className="text-[8.5px] font-mono font-bold truncate">Download PDF</span>
                                  </button>
                                </div>
                              </div>
                            )}
                      ...
                      <div className="glass-card p-6 space-y-4 flex flex-col justify-between">
                        <div>
                          ...
                          {getSkillsArray(resume?.parsedData?.skills).length > 0 ? (
                            <div className="flex flex-wrap gap-1.5 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
